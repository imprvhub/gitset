...
export async function revokeGithubToken(token: string): Promise<void> {
  const response = await fetch(`https://api.github.com/applications/${import.meta.env.GITHUB_AUTH_CLIENT}/token`, {
    method: 'DELETE',
    headers: {
        'Authorization': 'Basic ' + Buffer.from(`${import.meta.env.GITHUB_AUTH_CLIENT}:${import.meta.env.GITHUB_AUTH_SECRET}`).toString('base64'),
